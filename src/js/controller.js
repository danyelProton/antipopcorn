...
import { LUMIERE_URL, FILM_EUROPE_URL, NOSTALGIA_URL, MLADOST_URL, LUMIERE_ID, FILM_EUROPE_ID, NOSTALGIA_ID, MLADOST_ID, RESULTS_PER_PAGE } from './config.js';
...
const controlCinemaProgram = async function() {
  try {
    ...
    await model.getProgramLumiereFilmEurope(FILM_EUROPE_URL, FILM_EUROPE_ID);
...
const controlMovieDetails = async function(movie, render = true) {
  try {
    ...
    // get movie details (movie page), parameters are html movie data attributes
...
const controlPagination = function() {
  ...
  const firstOnPage = document.querySelectorAll('.first');

  // scroll last (most recent) 'first' movie into view - active movie (if there's one) is not collapsed, therefore no timeout
  firstOnPage[firstOnPage.length - 1].scrollIntoView({behavior: 'smooth'});
